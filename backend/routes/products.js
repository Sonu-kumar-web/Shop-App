...
router.get("/", (req, res, next) => {
   ...
   // Connect to mongodb by Atlas Cluster
   // MongoClient.connect(
   //    "mongodb+srv://sonu:sonu@free-cluster-t4e4s.mongodb.net/shop?retryWrites=true&w=majority"
   // )
   //    .then((client) => {
   ...
   db.getDb()
      ...
      .collection("product")
      .find()
      // These three lines can be use for pagination (sort, skip and limit)
      // .sort({ price: -1 })
      // .skip((queryPage - 1) * pageSize)
      // .limit(pageSize)
      .forEach((productDoc) => {
         // As find method returns cursor
